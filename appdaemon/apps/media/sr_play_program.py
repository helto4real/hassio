...
#
# See
# http://api.sr.se/api/v2/episodes/getlatest?program_id=4540&format=json&ondemandaudiotemplateid=8
#


class SRPlayProgram(hass.Hass):

    def initialize(self) -> None:
        """Initializer"""
        self.log("APP_SR_PROGRAM setting upp listen to event CMD_SR_PLAY_PROGRAM ")
        self.listen_event(self.__on_cmd_sr_play_program,
                          event='CMD_SR_PLAY_PROGRAM')

    def __on_cmd_sr_play_program(self, event_name, data, kwargs) -> None:
        """We got an event to play a program"""
        program_id = data.get('program_id')  # "4540"
        entity_id = data.get('entity_id')

        response = urlopen(
            'http://api.sr.se/api/v2/episodes/getlatest?format=json&programid=' + program_id)
